...
async function execute(_input: Record<string, unknown>, ctx: ToolContext): Promise<ToolResult> {
  ...
  const [
    ...
    run('hostname', 5_000),
    run('cat /etc/os-release 2>/dev/null | grep -E "^PRETTY_NAME=" | cut -d= -f2- | tr -d \'"\'', 5_000),
    run('uname -r', 5_000),
    run('uptime -p 2>/dev/null || uptime', 5_000),
    run('cat /proc/loadavg', 5_000),
    run('nproc', 5_000),
    run('grep -m1 "^model name" /proc/cpuinfo | cut -d: -f2- | sed \'s/^ //\'', 5_000),
    run('free -b', 5_000),
    run('df -PB1 -x tmpfs -x devtmpfs -x squashfs', 8_000),
    run('ps -e --no-headers | wc -l', 5_000),
    run('ss -Hltn 2>/dev/null || ss -ltn', 8_000),
    run('ss -Hlun 2>/dev/null || ss -lun', 8_000),
  ]);
